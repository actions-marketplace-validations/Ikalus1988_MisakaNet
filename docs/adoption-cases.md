# MisakaNet 采纳场景（内部测量与内部推演）

> **口径说清楚（2026-09-24）**：下面两个场景没有外部公司名、没有日期、也没有可回查的联系人——
> 它们**不是第三方核验过的客户案例**，数字是**内部使用与内部推演**的口径，匿名反馈也只是要点摘录。
> 采购评审请按"我们自己估计的效果"读，不要当成参考客户证据：本仓
> `docs/maintainer/blueprint-and-strategy-review-2026-09-16.md` §4.3 的自评是同一个口径
> （对大公司和机构：无试点、无合规材料，0.5 / 1）。等有第一个**外部部署并同意署名**，
> 这里才会出现真正的案例——在那之前"案例"二字不该出现。

---

## 场景 1（内部使用）：某工业机器人集成商的 RAG 知识库
